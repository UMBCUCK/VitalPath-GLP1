...
function PriceInput({ label, value, onChange, placeholder, required }: {
  label: string; value: number | null | undefined;
  onChange: (v: number | null) => void; placeholder?: string; required?: boolean;
}) {
    ...
    <div className={cn(
      ...
      <div className={cn("px-6 pb-4", state.badge ? "pt-9" : "pt-6")}>
        ...
        <div className="mt-4">
          <div className="flex items-baseline gap-1.5">
            <span className="text-4xl font-bold text-navy tracking-tight">{formatPrice(state.priceMonthly)}</span>
          ...
          {state.priceAnnual && (
            <p className="mt-0.5 text-xs text-graphite-400">
              or {formatPrice(Math.round(state.priceAnnual / 12))}/mo billed annually
            </p>
          )}
        </div>
      </div>
      ...
      <div className="px-6 pb-6 pt-2 space-y-3">
        <button className={cn(
          ...
          state.isFeatured ? "bg-navy text-white" : "border border-navy/30 text-navy hover:bg-navy-50"
        )}>
...
function PlanEditorPane({ product, allProducts, onCancel, onSaved }: {
  ...
  const otherPlans = allProducts.filter((p) => p.type === "MEMBERSHIP" && p.id !== product.id);
  ...
  return (
    <div className="border-t border-navy-100/40 bg-white">
      ...
      {/* Mobile preview strip (visible below lg) */}
      {showPreview && (
        <div className="lg:hidden border-b border-navy-100/40 bg-gradient-to-r from-navy-50/60 to-transparent px-5 py-4">
          <p className="mb-3 flex items-center gap-1.5 text-xs font-semibold text-navy">
        ...
        <div className="overflow-y-auto p-5 space-y-6" style={{ maxHeight: "72vh" }}>
          ...
          <section>
            ...
            {state.priceMonthly > 0 && (
              <div className="mt-2 flex flex-wrap gap-4 text-[10px] text-graphite-400">
                <span>Daily cost: <span className="font-semibold text-navy">${(state.priceMonthly / 100 / 30).toFixed(2)}/day</span></span>
                {state.priceAnnual && (
                  <span>Annual monthly equiv: <span className="font-semibold text-navy">{formatPrice(Math.round(state.priceAnnual / 12))}/mo</span></span>
                )}
                {state.priceAnnual && (
                  <span>Annual savings vs monthly: <span className="font-semibold text-emerald-600">{formatPrice(state.priceMonthly * 12 - state.priceAnnual)}/yr</span></span>
                )}
              </div>
            )}
          </section>
        ...
        {showPreview && (
          <div className="hidden lg:block border-l border-navy-100/40 bg-gradient-to-b from-navy-50/40 to-transparent p-5">
            <div className="mb-4 flex items-center gap-2">
              ...
              <span className="text-[10px] text-graphite-300">— matches public pricing page</span>
            </div>
...
// ─── Add-on editor (inline, no preview card) ─────────────────────────────────
              ...
              <div className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-graphite-400">{p.slug} · {Array.isArray(p.features) ? (p.features as unknown[]).length : 0} features</span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
              ...
              <div key={p.id} className={cn(
                ...
                <div className="flex items-start justify-between p-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {p.description && <p className="mt-1 text-xs text-graphite-500 line-clamp-2">{p.description}</p>}
                    <p className="mt-1.5 text-sm font-bold text-teal">{formatPrice(p.priceMonthly)}/mo</p>
                    {p.stripePriceIdMonthly
                  ...
                  <div className="flex items-center gap-2 ml-3 shrink-0">
                    {editingId !== p.id && (
                      <button onClick={() => startEdit(p)}
                        className="rounded-md border border-navy-100/60 p-1.5 text-graphite-400 hover:border-teal hover:text-teal transition-all">
